//! A tagged-pointer abstraction that allows size styles in Caelum to be represented
...
impl CaelumZero for CompactLength {
    const ZERO: Self = Self::length(0.0);
}
impl CaelumAuto for CompactLength {
    const AUTO: Self = Self::auto();
}
impl CaelumMinContent for CompactLength {
    const MIN_CONTENT: Self = Self::min_content();
}
impl CaelumMaxContent for CompactLength {
...
impl CaelumFitContent for CompactLength {
