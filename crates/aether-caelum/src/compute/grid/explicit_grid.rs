...
use crate::style_helpers::CaelumAuto;
...
pub(crate) fn compute_explicit_grid_size_in_axis(
    ...
    let repetition_definition = template
        ...
        .expect("auto-repeat def must exist when auto_repetition_count > 0");
    ...
    let num_repetitions: u16 = match auto_fit_container_size {
        ...
        Some(inner_container_size) => {
            ...
            fn track_definite_value(
                ...
                max_size.map(|max| max.maybe_max(min_size)).or(min_size).unwrap_or(0.0)
            }
...
pub(super) fn initialize_grid_tracks(
    ...
    if let Some(first) = tracks.first_mut() { first.collapse(); }
    if let Some(last) = tracks.last_mut() { last.collapse(); }
}
...
fn create_implicit_tracks(
    ...
    for _ in 0..count {
        let track_def = auto_tracks_iter.next().expect("infinite cycle/repeat iterator");
