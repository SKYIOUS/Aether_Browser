...
use vayu_browser::engine::net::cookies::{
    default_path, domain_match, parse_set_cookie, path_match, registrable_site, same_site_allowed,
    schemeful_site, SameSite,
};
...
fn set(
    ...
    path: &str,
    header: &str,
) -> Option<vayu_browser::engine::net::cookies::CookieRecord> {
...
fn c2_samesite_none_requires_secure() {
    assert!(
        set("example.com", "/", "sid=1; SameSite=None").is_none(),
        "None without Secure must drop"
    );
...
fn c2_cookie_line_size_boundary_4096_exact() {
    ...
    assert!(
        set("example.com", "/", &at_limit).is_some(),
        "4096 bytes accepted"
    );
    let over = make(4097);
    assert_eq!(over.len(), 4097);
    assert!(
        set("example.com", "/", &over).is_none(),
        "4097 bytes rejected"
    );
}
...
fn c2_path_match_honors_directory_boundaries() {
    ...
    assert!(
        !path_match("/foobar", "/foo"),
        "/foo must NOT match /foobar"
    );
    assert!(path_match("/foo", "/foo"));
    assert!(
        path_match("/foo/", "/foo"),
        "trailing-slash request matches its own dir"
    );
    assert!(path_match("/", "/"));
}
...
fn c2_send_matrix_full_table() {
    ...
    let cases: &[(SameSite, Option<&str>, bool, bool, &str)] = &[
        ...
        (
            ...
            true,
            false,
            "strict blocked even on cross-site nav",
        ),
        (
            ss_none,
            Some("https://evil.com"),
            false,
            true,
            "None travels anywhere (set-time Secure guard applies)",
        ),
        (
            Lax,
            None,
            true,
            true,
            "document navigation without initiator",
        ),
    ];
    for &(ss, initiator, top, expected, why) in cases {
        assert_eq!(
            same_site_allowed(
                ss,
                initiator.map(str::to_string).as_deref(),
                "https://site.com",
                top
            ),
            expected,
...
fn c2_schemeful_site_uses_registrable_domain_and_scheme() {
    assert_eq!(
        schemeful_site("https://a.app.example.com:8443/x"),
        "https://example.com"
    );
    assert_eq!(schemeful_site("http://EXAMPLE.com/"), "http://example.com");
    assert_eq!(
        schemeful_site("https://localhost:3000"),
        "https://localhost"
    );
    assert_ne!(
