...
fn test_tab_struct_construction() {
    let tab = Tab::new("Home", "https://example.com");
...
#[test]
fn test_tab_clone() {
    let tab = Tab::new("Test", "https://rust-lang.org");
...
fn test_tab_serialization_roundtrip() {
    let tabs = vec![
        Tab::new("Tab 1", "https://a.com"),
        Tab::new("Tab 2", "https://b.com"),
    ];
...
fn test_save_tabs_multiple() {
    ...
    let tabs = vec![
        Tab::new("Rust", "https://rust-lang.org"),
        Tab::new("Iced", "https://iced.rs"),
    ];
    let json = serde_json::to_string(&tabs).unwrap();
    std::fs::write(path, &json).unwrap();
    let loaded: Vec<Tab> = serde_json::from_str(&std::fs::read_to_string(path).unwrap()).unwrap();
...
fn test_tab_title_update() {
    let mut tab = Tab::new("Loading...", "https://example.com");
