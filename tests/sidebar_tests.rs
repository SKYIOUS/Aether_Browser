...
fn test_tab_struct_construction() {
    let tab = Tab::new("Home", "https://example.com", 0);
...
#[test]
fn test_tab_clone() {
    let tab = Tab::new("Test", "https://rust-lang.org", 0);
...
fn test_tab_serialization_roundtrip() {
    let tabs = vec![
        Tab::new("Tab 1", "https://a.com", 0),
        Tab::new("Tab 2", "https://b.com", 0),
    ];
...
fn test_save_tabs_multiple() {
    let path = "vayu_tabs_test2.json";
    let tabs = vec![
        Tab::new("Rust", "https://rust-lang.org", 0),
        Tab::new("Iced", "https://iced.rs", 0),
    ];
    let json = serde_json::to_string(&tabs).unwrap();
    std::fs::write(path, &json).unwrap();
    let loaded: Vec<Tab> = serde_json::from_str(&std::fs::read_to_string(path).unwrap()).unwrap();
...
fn test_tab_title_update() {
    let mut tab = Tab::new("Loading...", "https://example.com", 0);
