...
| 2 | ~~HTTP cache is an unbounded `HashMap`; TTL checked on read, expired entries never evicted, no byte/entry cap~~ **FIXED** | `src/engine/net/mod.rs:126-157` | Replaced `HashMap<String, (String, Instant)>` with `lru::LruCache<String, (String, Instant)>` (capacity 256). LRU eviction prevents unbounded memory growth. TTL preserved (60s check on read). Expired entries still detected on read but capacity is now bounded. Image cache (line 1095) remains unbounded — separate item if needed. | **FIXED** — LRU entry-count bound (256), `tests/http_cache.rs` regression coverage |
...
| 12 | ~~CSS shorthand `border:` not expanded by aether-css resolver~~ **FIXED** | `crates/aether-css/src/resolver.rs` | Longhand `border-*-width`/`border-*-color` handled but `border:` shorthand ignored; Stratus received zero widths | **FIXED** — `apply_border_shorthand()` added; 11 unit tests; `border-style` keywords silently ignored (unsupported) |
...
| 19 | ~~Production poisoned-lock panics (6 sites)~~ **FIXED** | `src/engine/text.rs:121`, `src/engine/pipeline/layout.rs:30,39,49`, `src/engine/pipeline/fetcher.rs:977,1127` | Mutex/RwLock `.unwrap()` on poisoned locks and `.join().expect()` on panicked threads crashed the browser process. Recovery: `unwrap_or_else(\|e\| e.into_inner())` for locks (safe — cosmic-text/fonts/caches tolerate stale state); `join().unwrap_or_else(\|_\| default)` for thread joins (safe — missed resource logged, existing error path handles it) | **FIXED** — 6 sites; 4 regression tests in `tests/poison_recovery.rs`; `cargo fmt` + `cargo clippy -- -D warnings` + workspace tests clean |
