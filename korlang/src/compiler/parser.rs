...
#[derive(Debug, Clone)]
pub enum Expr { Literal(String), Number(f64), Identifier(String), Binding(String) }
#[derive(Debug, Clone)]
pub struct Property { pub name: String, pub value: Expr }
#[derive(Debug, Clone)]
pub struct Element { pub name: String, pub properties: Vec<Property>, pub children: Vec<Element>, pub on_click: Option<String> }
#[derive(Debug, Clone)]
pub struct State { pub name: String, pub type_name: String, pub default_value: Expr }
#[derive(Debug, Clone)]
pub struct Component { pub name: String, pub states: Vec<State>, pub root: Element }
    ...
    fn parse_element(&mut self) -> Option<Element> {
        ...
        if self.consume(Token::OpenBrace) {
            while self.peek() != Some(&Token::CloseBrace) && !self.is_eof() {
                ...
                if let Some(child) = self.parse_element() { children.push(child); } else { break; }
            }
            self.expect(Token::CloseBrace);
        }
