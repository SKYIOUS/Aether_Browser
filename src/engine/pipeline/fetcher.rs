...
use super::extractor::{
    extract_elements_flat, BoxSizing, FontWeight, StyledElement, TextDecor, MAX_ELEMENTS,
};
...
) -> (String, Vec<StyledElement>, Option<Arc<Mutex<JsBridge>>>) {
    let page = match url {
        ...
        _ => {
            return (
                url.to_string(),
                error_page(url, "Unknown internal page", content_width, viewport_h, 0),
                None,
            )
        }
    };
...
fn newtab_page(content_width: f32, viewport_h: f32) -> Vec<StyledElement> {
    let se = |tag: &str,
              ...
              size: f32,
              weight: &str,
              bg: Option<iced::Color>| StyledElement {
        tag: tag.into(),
        text: text.into(),
        wrapped_lines: vec![],
        dom_path: vec![],
...
fn history_page(
    ...
    let se = |text: &str,
              ...
              weight: &str,
              is_link: bool,
              href: Option<String>| StyledElement {
        ...
        flex_shrink: 0.0,
        flex_basis: None,
        css_width: Some(content_width - pad * 2.0),
        css_height: None,
        parent_index: Some(0),
        min_width: None,
        max_width: None,
...
fn error_page(
    ...
    let (red, title) = match status {
        404 => (
            Color::from_rgb(0.88, 0.18, 0.18),
            "404 - Not Found".to_string(),
        ),
        403 => (
            Color::from_rgb(0.88, 0.55, 0.18),
...
pub async fn fetch_page_content(
    ...
    tokio::task::spawn_blocking(move || {
        do_fetch_page_content_sync(url, content_width, viewport_h, session_history)
    })
    .await
    .unwrap_or_else(|e| {
...
fn do_fetch_page_content_sync(
    ...
    let response = match net::fetch_with_redirects(&url, 5, None, None, true) {
        ...
        Err(e) => {
            ...
            return (
                url.clone(),
                error_page(&url, &reason, content_width, viewport_h, status),
                None,
            );
        }
    };
    ...
    if inline_styles_ok {
        let inline_refs: Vec<&str> = styles
            ...
            .map(|s| {
                if s.len() > MAX_CSS_SOURCE_BYTES {
                    plog!(
                        "CSS",
                        "Trimmed inline style from {} to {} bytes",
                        s.len(),
            ...
            .collect();
        let (kept, used) =
            css_sources_within_total_budget(&inline_refs, css_used, CSS_TOTAL_BUDGET_BYTES);
        if kept.len() < inline_refs.len() {
            plog!(
                "CSS",
                "Total-CSS budget skipped {} inline style block(s)",
                inline_refs.len() - kept.len()
            );
        }
        for (si, trimmed) in kept.iter().enumerate() {
            ...
            plog!(
                "STYLE",
                "Parsed {} rules from inline style {}",
                rules.len(),
                si
            );
        ...
        plog!(
            "CSP",
            "Blocked all {} inline style block(s) (no 'unsafe-inline')",
            styles.len()
        );
    }
    plog!(
        "CSS",
        "{} rules from inline styles, {} bytes of budget used",
    ...
    if !css_fetch_set.is_empty() {
        ...
        let url_ref = &url;
        let results: Vec<(String, std::result::Result<Vec<u8>, ()>)> =
            std::thread::scope(|scope| {
                let handles: Vec<_> = css_fetch_set
                    .iter()
                    .map(|resolved| {
                        let pu = page_url_ref;
                        let ur = url_ref;
                        scope.spawn(move || {
                            match net::fetch_resource(
                                resolved,
                                pu,
                                net::ResourceKind::Style,
                                Some(ur),
                                false,
                            ) {
                                Ok(resp) if resp.status < 400 => {
                                    (resolved.clone(), Ok(resp.body.into_bytes()))
                                }
                                Ok(resp) => {
                                    plog!(
                                        "CSS",
                                        "External CSS HTTP error {} for {}",
                                        resp.status,
                                        resolved
                                    );
                                    (resolved.clone(), Err(()))
                                ...
                                Err(e) => {
                                    plog!("CSS", "Failed to fetch external CSS: {}", e);
                                    (resolved.clone(), Err(()))
                                }
                            }
                        })
        ...
        for (resolved, body_result) in results {
            ...
            if css_content.len() > MAX_CSS_SOURCE_BYTES {
                plog!(
                    "CSS",
                    "Trimmed external CSS from {} to {} bytes",
                    css_content.len(),
                    MAX_CSS_SOURCE_BYTES
                );
            }
            let trimmed = trim_to_budget(&css_content, MAX_CSS_SOURCE_BYTES);
            let (kept, used_after) = css_sources_within_total_budget(
                std::slice::from_ref(&trimmed),
                css_used,
                CSS_TOTAL_BUDGET_BYTES,
            );
            if kept.is_empty() {
                plog!(
                    "CSS",
                    "Total-CSS budget skipped external sheet {}",
                    resolved
                );
                continue;
            }
    ...
    plog!(
        "JS",
        "Found {} script blocks (js_enabled={})",
        scripts.len(),
        js_enabled
    );
    ...
    let mut img_cache: LruCache<String, (f32, f32, Handle)> =
    ...
    for el in elements.iter_mut() {
        let Some(ref img_src) = el.image_url else {
            continue;
        };
        let resolved = net::resolve_url(img_src, &url);
        if let Some((w, hh, hnd)) = img_cache
            .get(&resolved)
            .map(|(w, h, h2)| (*w, *h, h2.clone()))
        {
            el.width = w;
            el.height = hh;
        ...
        let Some(bytes) = fetched_map.get(&resolved) else {
            continue;
        };
        if bytes.len() >= 5_000_000 {
            plog!(
                "IMAGES",
                "Image too large ({} bytes), skipping decode",
                bytes.len()
            );
            continue;
        }
            ...
            if is_svg_bytes(bytes) {
                decode_svg(bytes)
                    .map(|rgba| {
                        ...
                        let scale = if (w as f32).max(h as f32) > 800.0 {
                            800.0 / (w as f32).max(h as f32)
                        } else {
                            1.0
                        };
                        if scale < 1.0 {
                            ...
                            (
                                rw as f32,
                                rh as f32,
                                Handle::from_rgba(rw, rh, resized.into_raw()),
                            )
                        } else {
        ...
        if let Some((fw, fh, handle)) = decode_and_resize(bytes) {
            ...
            plog!(
                "IMAGES",
                "Failed to decode image bytes ({} bytes)",
                bytes.len()
            );
        }
    }
...
fn decode_svg(bytes: &[u8]) -> Option<image::RgbaImage> {
    ...
    match rx.recv_timeout(std::time::Duration::from_millis(SVG_RENDER_TIMEOUT_MS)) {
        ...
        Err(_) => {
            plog!(
                "SVG",
                "SVG render timed out (>{})ms, skipping",
...
mod budget_tests {
    ...
    fn css_budget_counts_retained_bytes_not_original() {
        ...
        assert!(
            kept.is_empty(),
            "source larger than the whole budget must be skipped"
        );
        assert_eq!(total, 0);
    }
}
...
mod b3_history_tests {
    ...
    fn b3_display_trimmed_but_href_full() {
        ...
        assert!(
            entry.text.len() <= 64,
            "display text should be trimmed, got {}",
            entry.text.len()
        );
    }
    ...
    fn d2b_profile_font_and_layout() {
        use super::super::extractor::{BoxSizing, FontWeight, StyledElement, TextDecor};
        ...
        for i in 1..=5 {
            let t = Instant::now();
            let _w =
                measure_text_width(&format!("iteration {i} unique text for measurement"), 16.0);
            println!(
                "[d2b] measure_text_width #{:<2}      : {:?}",
                i,
                t.elapsed()
            );
        }
        ...
        for i in 1..=3 {
            ...
            let _w = measure_text_width("hello world", 16.0);
            println!(
                "[d2b] measure_text_width cached#{:<2}: {:?}",
                i,
                t.elapsed()
            );
        }
    ...
    fn d4_e2e_validation() {
        ...
        fn run_scenario(
            ...
            let mut m = mock::MockHttpResponder::new()
                .delay_ms(0)
                .html(&format!("mock://d4/{}", name), doc);
            ...
            let t = Instant::now();
            let r_async =
                rt.block_on(async { fetch_page_content(url.clone(), width, height, vec![]).await });
            let async_cold = t.elapsed();
            println!(
                "[d4] {} async cold: {:?} ({} elements)",
                name,
                async_cold,
                r_async.1.len()
            );
        ...
        let large_doc = {
            let mut s = String::from(
                "<html><head><link rel=stylesheet href=\"mock://d4/large/a.css\"></head><body>",
            );
            for i in 0..5000 {
        ...
        let deep_doc = {
            let mut s = String::from(
                "<html><head><link rel=stylesheet href=\"mock://d4/deep/a.css\"></head><body>",
            );
            for _ in 0..200 {
                s.push_str("<div>");
            }
        ...
        let multi_doc = {
            ...
            for c in 0..10 {
                s.push_str(&format!(
                    "<link rel=stylesheet href=\"mock://d4/multi/c{c}.css\">"
                ));
            }
            s.push_str("</head><body><p>multi resource</p>");
            for i in 0..20 {
                s.push_str(&format!("<img src=\"mock://d4/multi/i{i}\">"));
            }
            s.push_str("</body></html>");
            s
        };
        let multi_css: Vec<_> = (0..10)
            .map(|c| {
                (
                    format!("c{c}.css"),
                    format!(".c{c}{{color:hsl({}deg,100%,50%)}}", c * 36),
                )
            })
            .collect();
        ...
        println!(
            "[d4]   Fetch baseline: ~10.5ms (steady-state), ~380ms font init on true cold start"
        );
    ...
    fn d4_concurrency_validation() {
        ...
        let doc = {
            ...
            for c in 0..5 {
                s.push_str(&format!(
                    "<link rel=stylesheet href=\"mock://d4/conc/c{c}.css\">"
                ));
            }
            s.push_str("</head><body><p>concurrency test</p>");
            for i in 0..10 {
                s.push_str(&format!("<img src=\"mock://d4/conc/i{i}\">"));
            }
            s.push_str("</body></html>");
            s
        };
        let css: Vec<_> = (0..5)
            .map(|c| {
                (
                    format!("c{c}.css"),
                    format!(".c{c}{{color:hsl({}deg,100%,50%)}}", c * 72),
                )
            })
            .collect();
        ...
        let m = mock::MockHttpResponder::new()
            .delay_ms(5)
            .html("mock://d4/conc", &doc);
        let mut m = m;
        for (u, c) in &css {
            m = m.css(&format!("mock://d4/conc/{}", u), c);
        }
        for (u, d) in &img {
            m = m.binary(&format!("mock://d4/conc/{}", u), d.clone());
        }
        mock::set_mock(m);
        crate::engine::pipeline::set_js_enabled(false);
