...
impl TextDecor {
    ...
    fn from_css(value: Option<&str>) -> Self {
        ...
        if v.contains("overline") {
            d.0 |= TextDecor::OVERLINE.0;
        }
        if v.contains("line-through") {
            d.0 |= TextDecor::LINE_THROUGH.0;
        }
        d
    }
}
...
pub fn should_skip_content(tag: &str) -> bool {
    matches!(
        tag,
        "script" | "style" | "noscript" | "template" | "svg" | "title" | "iframe"
    )
}
...
fn default_top_margin(tag: &str) -> f32 {
    match tag {
        ...
        "img" => 4.0,
        "form" => 12.0,
        "button" | "input" | "textarea" | "select" => 4.0,
        _ => 0.0,
    }
}
...
fn apply_media_defaults(tag: &str, el: &mut StyledElement) {
    if tag == "img" {
        if el.css_width.is_none() {
            el.css_width = Some(200.0);
        }
        if el.css_height.is_none() {
            el.css_height = Some(150.0);
        }
    }
    if matches!(tag, "input" | "button" | "textarea" | "select") {
        ...
        if el.css_height.is_none() {
            el.css_height = Some(32.0);
        }
        if tag == "textarea" {
            el.css_height = Some(64.0);
        }
    }
}
...
fn compute_full_style_inner(
    ...
    let border_color = cs
        .border_top_color
        .or(cs.border_right_color)
        .or(cs.border_bottom_color)
        .or(cs.border_left_color)
        .map(|c| stratus_color(&c));
    ...
    let fs = FullStyle {
        ...
        align_content: cs.flex.align_content,
        box_sizing,
...
fn make_element(
    ...
    StyledElement {
        ...
        image_handle: None,
        image_url: None,
        margin_top: fs.margin_top,
        margin_bottom: fs.margin_bottom,
        margin_left: fs.margin_left,
        margin_right: fs.margin_right,
        padding: fs.padding,
        display: fs.display,
        flex_direction: fs.flex_direction,
        flex_wrap: fs.flex_wrap,
        justify_content: fs.justify_content,
        align_items: fs.align_items,
        align_self: fs.align_self,
        align_content: fs.align_content,
        box_sizing: fs.box_sizing,
        flex_grow: fs.flex_grow,
        flex_shrink: fs.flex_shrink,
        flex_basis: fs.flex_basis,
        css_width: fs.css_width,
        css_height: fs.css_height,
        min_width: fs.min_width,
        max_width: fs.max_width,
        min_height: fs.min_height,
        max_height: fs.max_height,
    ...
    match &node.node_type {
        ...
        NodeType::Text(text) => {
            ...
            if !txt.is_empty()
                ...
                && !txt.chars().all(|c| c.is_whitespace())
            {
        ...
        NodeType::Element(elem) => {
            ...
            let (
                ...
                "h1" => {
                    ...
                    if text.is_empty() {
                        ...
                        text_consumed = true;
                        (text, false, None, 0, "h1", false, false)
                    }
                }
                "h2" => {
                    let text = get_all_text(node);
                    if text.is_empty() {
                        ...
                        text_consumed = true;
                        (text, false, None, 0, "h2", false, false)
                    }
                }
                "h3" | "h4" | "h5" | "h6" => {
                    let text = get_all_text(node);
                    if text.is_empty() {
                        ...
                        text_consumed = true;
                        (text, false, None, 1, tag.as_str(), false, false)
                    }
                }
                "p" => {
                    ...
                    if direct_text.is_empty() {
                        ...
                        text_consumed = true;
                        (direct_text, false, None, 0, "p", false, true)
                    }
                }
                "li" => {
                    ...
                    if has_link {
                        ...
                        if text.is_empty() {
                            ...
                            text_consumed = true;
                            (format!("• {}", text), false, None, 1, "li", false, false)
                        }
                    }
                }
                "img" => {
            ...
            let needs_skip_children =
                !text_content.is_empty() && recurse_into_children && !skip_element;

            let this_idx = if !skip_element {
                let tag_name = if tag_override.is_empty() {
                    &tag
                } else {
                    tag_override
                };
                let mut el =
                    make_element(tag_name, text_content, &fs, parent_idx, dom_path.clone());
                el.is_link = is_link;
                el.href = href;
            ...
            let (new_parent, skip_fn): (Option<usize>, SkipFn) = match tag.as_str() {
                ...
                "a" | "h1" | "h2" | "h3" | "h4" | "h5" | "h6" | "textarea" => {
                    if text_consumed {
                        (
                            parent_idx,
                            Box::new(|c| matches!(c.node_type, NodeType::Text(_))),
                        )
                    } else {
                ...
                "p" => {
                    ...
                    (
                        new_p,
                        Box::new(|c: &Node| matches!(c.node_type, NodeType::Text(_))),
                    )
                }
...
fn get_all_text_flat(nodes: &[FlatNode], id: u32) -> String {
    fn collect(nodes: &[FlatNode], id: u32, out: &mut String) {
        let node = match nodes.get(id as usize) {
            Some(n) => n,
            None => return,
        };
        if node.is_text {
    ...
    fn walk(
        ...
        let (
            ...
            "h3" | "h4" | "h5" | "h6" => {
                ...
                if text.is_empty() {
                    ...
                    (text, false, None, 1, tag, false, false)
                }
            }
            "p" => {
            ...
            "li" => {
                let has_link = node.children.iter().any(|&c| {
                    nodes
                        .get(c as usize)
                        .is_some_and(|n| !n.is_text && n.tag == "a")
                });
                if has_link {
                    (String::new(), false, None, 0, "", true, false)
                } else {
            ...
            "input" => {
                ...
                let input_type = node
                    ...
                    .unwrap_or_else(|| "text".to_string());
                let text =
                    if input_type == "submit" || input_type == "button" || input_type == "reset" {
        ...
        let (new_parent, skip_text_children): (Option<usize>, bool) = match tag {
            ...
            _ => {
                if skip_element {
                    (parent_idx, false)
                } else if needs_skip_children {
                    (this_idx.or(parent_idx), true)
                } else {
                    (this_idx.or(parent_idx), false)
                }
            }
        };

        for (ci, &child_id) in node.children.iter().enumerate() {
            let child = match nodes.get(child_id as usize) {
                Some(c) => c,
                None => continue,
            };
            if skip_text_children && child.is_text {
                continue;
            }
            dom_path.push(ci);
            walk(
...
mod tests {
    ...
    fn test_extract_decodes_text() {
        ...
        crate::engine::pipeline::extractor::extract_elements(
            &dom,
            &mut elements,
            0,
            &sheet,
