...
impl DomSink {
    fn new() -> Self {
        let initial_names = vec![html5ever::QualName::new(
            None,
            "".into(),
            "".into(),
        )];
    ...
    fn store_qual_name(&self, handle: &Rc<RefCell<Node>>, name: html5ever::QualName) -> usize {
        ...
        self.handle_to_index.borrow_mut().insert(handle.as_ref() as *const _, idx);
        idx
    }
    
    fn add_child(&self, parent: &Rc<RefCell<Node>>, child: Rc<RefCell<Node>>) {
        self.children.borrow_mut().entry(parent.as_ref() as *const _).or_default().push(child);
    }
    
    fn build_tree(handle: Rc<RefCell<Node>>, children_map: &HashMap<*const std::cell::RefCell<Node>, Vec<Rc<RefCell<Node>>>>) -> Node {
        let mut node = (*handle).borrow().clone();
        let ptr = handle.as_ref() as *const _;
        if let Some(child_handles) = children_map.get(&ptr) {
            node.children = child_handles.iter().map(|h| Self::build_tree(h.clone(), children_map)).collect();
        }
        node
    }
}
...
impl TreeSink for DomSink {
    ...
    fn get_document(&self) -> Self::Handle { self.document.clone() }
    fn set_quirks_mode(&self, _: html5ever::tree_builder::QuirksMode) {}
    
    fn create_element(&self, name: html5ever::QualName, attrs: Vec<html5ever::tree_builder::Attribute>, _: html5ever::tree_builder::ElementFlags) -> Self::Handle {
        let tag_name = name.local.to_string();
        let attributes: std::collections::HashMap<String, String> = attrs.into_iter()
            .map(|a| (a.name.local.to_string(), a.value.to_string()))
            .collect();
        let node = Rc::new(RefCell::new(Node::new_element(tag_name, attributes, Vec::new())));
