...
pub fn evenly_sized_tracks<S: CheapCloneStr>(count: u16) -> Vec<GridTemplateComponent<S>> {
    use crate::engine::caelum::util::sys::new_vec_with_capacity;
...
pub const fn zero<T: CaelumZero>() -> T {
    T::ZERO
}

/// Trait to abstract over zero values
pub trait CaelumZero {
    /// The zero value for type implementing CaelumZero
    const ZERO: Self;
}
impl CaelumZero for f32 {
    const ZERO: f32 = 0.0;
}
impl<T: CaelumZero> CaelumZero for Option<T> {
    const ZERO: Option<T> = Some(T::ZERO);
}
impl<T: CaelumZero> CaelumZero for Point<T> {
    const ZERO: Point<T> = Point { x: T::ZERO, y: T::ZERO };
}
impl<T: CaelumZero> Point<T> {
...
impl<T: CaelumZero> CaelumZero for Line<T> {
    const ZERO: Line<T> = Line { start: T::ZERO, end: T::ZERO };
}
impl<T: CaelumZero> Line<T> {
...
impl<T: CaelumZero> CaelumZero for Size<T> {
    const ZERO: Size<T> = Size { width: T::ZERO, height: T::ZERO };
}
impl<T: CaelumZero> Size<T> {
...
pub const fn auto<T: CaelumAuto>() -> T {
    T::AUTO
}

/// Trait to abstract over auto values
pub trait CaelumAuto {
    /// The auto value for type implementing CaelumAuto
    const AUTO: Self;
}
impl<T: CaelumAuto> CaelumAuto for Option<T> {
    const AUTO: Option<T> = Some(T::AUTO);
}
impl<T: CaelumAuto> CaelumAuto for Point<T> {
    const AUTO: Point<T> = Point { x: T::AUTO, y: T::AUTO };
}
impl<T: CaelumAuto> Point<T> {
...
impl<T: CaelumAuto> CaelumAuto for Line<T> {
    const AUTO: Line<T> = Line { start: T::AUTO, end: T::AUTO };
}
impl<T: CaelumAuto> Line<T> {
...
impl<T: CaelumAuto> CaelumAuto for Size<T> {
    const AUTO: Size<T> = Size { width: T::AUTO, height: T::AUTO };
}
impl<T: CaelumAuto> Size<T> {
...
/// Returns the auto value for that type
pub const fn min_content<T: CaelumMinContent>() -> T {
    T::MIN_CONTENT
}

/// Trait to abstract over min_content values
pub trait CaelumMinContent {
    /// The min_content value for type implementing CaelumZero
    const MIN_CONTENT: Self;
}
impl<T: CaelumMinContent> CaelumMinContent for Option<T> {
    const MIN_CONTENT: Option<T> = Some(T::MIN_CONTENT);
}
impl<T: CaelumMinContent> CaelumMinContent for Point<T> {
    const MIN_CONTENT: Point<T> = Point { x: T::MIN_CONTENT, y: T::MIN_CONTENT };
}
impl<T: CaelumMinContent> Point<T> {
...
impl<T: CaelumMinContent> CaelumMinContent for Line<T> {
    const MIN_CONTENT: Line<T> = Line { start: T::MIN_CONTENT, end: T::MIN_CONTENT };
}
impl<T: CaelumMinContent> Line<T> {
...
impl<T: CaelumMinContent> CaelumMinContent for Size<T> {
    const MIN_CONTENT: Size<T> = Size { width: T::MIN_CONTENT, height: T::MIN_CONTENT };
}
impl<T: CaelumMinContent> Size<T> {
...
/// Returns the auto value for that type
pub const fn max_content<T: CaelumMaxContent>() -> T {
    T::MAX_CONTENT
}

/// Trait to abstract over max_content values
pub trait CaelumMaxContent {
    /// The max_content value for type implementing CaelumZero
    const MAX_CONTENT: Self;
}
impl<T: CaelumMaxContent> CaelumMaxContent for Option<T> {
    const MAX_CONTENT: Option<T> = Some(T::MAX_CONTENT);
}
impl<T: CaelumMaxContent> CaelumMaxContent for Point<T> {
    const MAX_CONTENT: Point<T> = Point { x: T::MAX_CONTENT, y: T::MAX_CONTENT };
}
impl<T: CaelumMaxContent> Point<T> {
...
impl<T: CaelumMaxContent> CaelumMaxContent for Line<T> {
    const MAX_CONTENT: Line<T> = Line { start: T::MAX_CONTENT, end: T::MAX_CONTENT };
}
impl<T: CaelumMaxContent> Line<T> {
...
impl<T: CaelumMaxContent> CaelumMaxContent for Size<T> {
    const MAX_CONTENT: Size<T> = Size { width: T::MAX_CONTENT, height: T::MAX_CONTENT };
}
impl<T: CaelumMaxContent> Size<T> {
...
impl<T: CaelumFitContent> CaelumFitContent for Line<T> {
    fn fit_content(argument: LengthPercentage) -> Self {
        Line { start: T::fit_content(argument), end: T::fit_content(argument) }
    }
}
impl<T: CaelumFitContent> Line<T> {
...
impl<T: CaelumFitContent> CaelumFitContent for Size<T> {
    fn fit_content(argument: LengthPercentage) -> Self {
        Size { width: T::fit_content(argument), height: T::fit_content(argument) }
    }
}
impl<T: CaelumFitContent> Size<T> {
...
impl<T: CaelumFitContent> CaelumFitContent for Rect<T> {
    fn fit_content(argument: LengthPercentage) -> Self {
        Rect {
...
impl<T: CaelumFitContent> Rect<T> {
...
#[cfg(test)]
mod repeat_fn_tests {
    type S = crate::engine::caelum::util::sys::DefaultCheapStr;
    use super::repeat;
    use crate::engine::caelum::{
