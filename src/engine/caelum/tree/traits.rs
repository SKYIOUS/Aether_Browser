//! The abstractions that make up the core of Caelum's low-level API
//!
//! ## Examples
//!
//! The following examples demonstrate end-to-end implementation of Caelum's traits and usage of the low-level compute APIs:
//!
//!   - [custom_tree_vec](https://github.com/DioxusLabs/Caelum/blob/main/examples/custom_tree_vec.rs) which implements a custom Caelum tree using a `Vec` as an arena with NodeId's being index's into the Vec.
//!   - [custom_tree_owned_partial](https://github.com/DioxusLabs/Caelum/blob/main/examples/custom_tree_owned_partial.rs) which implements a custom Caelum tree using directly owned children with NodeId's being index's into vec on parent node.
//!   - [custom_tree_owned_unsafe](https://github.com/DioxusLabs/Caelum/blob/main/examples/custom_tree_owned_unsafe.rs) which implements a custom Caelum tree using directly owned children with NodeId's being pointers.
//!
//! ## Overview
//!
...
//! | ---                   | ---                     | ---                                                                                                                                                                                                                                                                                                                                                                                                                       |
//! | [`LayoutPartialTree`] | [`TraversePartialTree`] | [`compute_flexbox_layout`](crate::engine::caelum::compute_flexbox_layout)<br />[`compute_grid_layout`](crate::engine::caelum::compute_grid_layout)<br />[`compute_block_layout`](crate::engine::caelum::compute_block_layout)<br />[`compute_root_layout`](crate::engine::caelum::compute_root_layout)<br />[`compute_leaf_layout`](crate::engine::caelum::compute_leaf_layout)<br />[`compute_hidden_layout`](crate::engine::caelum::compute_hidden_layout)<br />[`compute_cached_layout`](crate::engine::caelum::compute_cached_layout) |
//! | [`RoundTree`]         | [`TraverseTree`]        | [`round_layout`](crate::engine::caelum::round_layout)                                                                                                                                                                                                                                                                                                                                                                                     |
//! | [`PrintTree`]         | [`TraverseTree`]        | [`print_tree`](crate::engine::caelum::print_tree)                                                                                                                                                                                                                                                                                                                                                                                         |
//!
//! ## All of the traits on one page
//!
//! ### TraversePartialTree and TraverseTree
//! These traits are Caelum's abstraction for downward tree traversal:
...
//!    for use of Caelum's core layout algorithms (flexbox, grid, etc).
...
//! ```rust
//! # use aether_browser::engine::caelum::*;
...
//! ```
//!
//! You must implement [`TraversePartialTree`] to access any of Caelum's low-level API. If your tree implementation allows you to implement [`TraverseTree`] with
//! the correct semantics (full recursive traversal is available) then you should.
//!
//! ### LayoutPartialTree
//!
//! **Requires:** `TraversePartialTree`<br />
//! **Enables:** Flexbox, Grid, Block and Leaf layout algorithms from the [`crate::engine::caelum::compute`] module
//!
//! Any type that implements [`LayoutPartialTree`] can be laid out using [Caelum's algorithms](crate::engine::caelum::compute)
//!
//! Note that this trait extends [`TraversePartialTree`] (not [`TraverseTree`]). Caelum's algorithm implementations have been designed such that they can be used for a laying out a single
...
use crate::engine::caelum::geometry::{AbsoluteAxis, Line, Size};
use crate::engine::caelum::style::{AvailableSpace, CoreStyle};
use crate::engine::caelum::style::{FlexboxContainerStyle, FlexboxItemStyle};
use crate::engine::caelum::style::{GridContainerStyle, GridItemStyle};
use crate::engine::caelum::CheapCloneStr;
use crate::engine::caelum::{BlockContainerStyle, BlockContext, BlockItemStyle};

use crate::engine::caelum::compute::grid::DetailedGridInfo;

/// Caelum's abstraction for downward tree traversal.
///
