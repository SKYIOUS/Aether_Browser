//! Contains [CaelumTree](crate::engine::caelum::tree::CaelumTree): the default implementation of [LayoutTree](crate::engine::caelum::tree::LayoutTree), and the error type for Caelum.
use slotmap::SparseSecondaryMap as SecondaryMap;
use slotmap::{DefaultKey, SlotMap};

use crate::engine::caelum::block::BlockContext;
use crate::engine::caelum::geometry::Size;
use crate::engine::caelum::style::{AvailableSpace, Display, Style};
use crate::engine::caelum::util::sys::DefaultCheapStr;
use crate::engine::caelum::tree::{
    Cache, ClearState, Layout, LayoutInput, LayoutOutput, LayoutPartialTree, NodeId, PrintTree, RoundTree, RunMode,
    TraversePartialTree, TraverseTree,
};
use crate::engine::caelum::util::sys::{new_vec_with_capacity, ChildrenVec, Vec};

use crate::engine::caelum::compute::{
    compute_cached_layout, compute_hidden_layout, compute_leaf_layout, compute_root_layout, round_layout,
};
use crate::engine::caelum::CacheTree;

use crate::engine::caelum::{compute::compute_block_layout, LayoutBlockContainer};
use crate::engine::caelum::{compute::compute_flexbox_layout, LayoutFlexboxContainer};
use crate::engine::caelum::{compute::compute_grid_layout, LayoutGridContainer};

use crate::engine::caelum::compute::grid::DetailedGridInfo;
use crate::engine::caelum::tree::layout::DetailedLayoutInfo;

/// The error Caelum generates on invalid operations
pub type CaelumResult<T> = Result<T, CaelumError>;
...
pub enum CaelumError {
    ...
    /// The parent node was not found in the [`CaelumTree`](crate::engine::caelum::CaelumTree) instance.
    InvalidParentNode(NodeId),
    /// The child node was not found in the [`CaelumTree`](crate::engine::caelum::CaelumTree) instance.
    InvalidChildNode(NodeId),
    /// The supplied node was not found in the [`CaelumTree`](crate::engine::caelum::CaelumTree) instance.
    InvalidInputNode(NodeId),
}

impl core::fmt::Display for CaelumError {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        match self {
            CaelumError::ChildIndexOutOfBounds { parent, child_index, child_count } => {
                write!(f, "Index (is {child_index}) should be < child_count ({child_count}) for parent node {parent:?}")
            }
            CaelumError::InvalidParentNode(parent) => {
                write!(f, "Parent Node {parent:?} is not in the CaelumTree instance")
            }
            CaelumError::InvalidChildNode(child) => write!(f, "Child Node {child:?} is not in the CaelumTree instance"),
            CaelumError::InvalidInputNode(node) => write!(f, "Supplied Node {node:?} is not in the CaelumTree instance"),
        }
    }
}
...
/// Global configuration values for a CaelumTree instance
#[derive(Debug, Clone, Copy)]
pub(crate) struct CaelumConfig {
    /// Whether to round layout values
    pub(crate) use_rounding: bool,
}

impl Default for CaelumConfig {
    fn default() -> Self {
        Self { use_rounding: true }
    }
}

/// Layout information for a given [`Node`](crate::engine::caelum::node::Node)
///
/// Stored in a [`CaelumTree`].
...
struct NodeData {
    ...
    /// layout to avoid errors from rounding already-rounded values. See <https://github.com/DioxusLabs/Caelum/issues/501>.
...
/// An entire tree of UI nodes. The entry point to Caelum's high-level API.
///
/// Allows you to build a tree of UI nodes, run Caelum's layout algorithms over that tree, and then access the resultant layout.]
#[derive(Debug, Clone)]
pub struct CaelumTree<NodeContext = ()> {
...
impl Default for CaelumTree {
    fn default() -> CaelumTree<()> {
        CaelumTree::new()
    }
}

/// Iterator that wraps a slice of nodes, lazily converting them to u64
pub struct CaelumTreeChildIter<'a>(core::slice::Iter<'a, NodeId>);
impl Iterator for CaelumTreeChildIter<'_> {
    type Item = NodeId;
...
// TraversePartialTree impl for CaelumTree
impl<NodeContext> TraversePartialTree for CaelumTree<NodeContext> {
    type ChildIter<'a>
        = CaelumTreeChildIter<'a>
    where
        Self: 'a;

    #[inline(always)]
    fn child_ids(&self, parent_node_id: NodeId) -> Self::ChildIter<'_> {
        CaelumTreeChildIter(self.children[parent_node_id.into()].iter())
    }
...
// TraverseTree impl for CaelumTree
impl<NodeContext> TraverseTree for CaelumTree<NodeContext> {}

// CacheTree impl for CaelumTree
impl<NodeContext> CacheTree for CaelumTree<NodeContext> {
...
// PrintTree impl for CaelumTree
impl<NodeContext> PrintTree for CaelumTree<NodeContext> {
    #[inline(always)]
    fn get_debug_label(&self, node_id: NodeId) -> &'static str {
        ...
        match (num_children, display) {
            ...
            (_, Display::Flex) => {
                use crate::engine::caelum::FlexDirection;
...
/// View over the Caelum tree that holds the tree itself along with a reference to the context
/// and implements LayoutTree. This allows the context to be stored outside of the CaelumTree struct
/// which makes the lifetimes of the context much more flexible.
pub(crate) struct CaelumView<'t, NodeContext, MeasureFunction>
where
    MeasureFunction:
...
impl<NodeContext, MeasureFunction> CaelumView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
...
// TraversePartialTree impl for CaelumView
impl<NodeContext, MeasureFunction> TraversePartialTree for CaelumView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
...
// TraverseTree impl for CaelumView
impl<NodeContext, MeasureFunction> TraverseTree for CaelumView<'_, NodeContext, MeasureFunction> where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>
{
}

// LayoutPartialTree impl for CaelumView
impl<NodeContext, MeasureFunction> LayoutPartialTree for CaelumView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
...
impl<NodeContext, MeasureFunction> CacheTree for CaelumView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
...
impl<NodeContext, MeasureFunction> LayoutBlockContainer for CaelumView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
...
impl<NodeContext, MeasureFunction> LayoutFlexboxContainer for CaelumView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
...
impl<NodeContext, MeasureFunction> LayoutGridContainer for CaelumView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
...
// RoundTree impl for CaelumView
impl<NodeContext, MeasureFunction> RoundTree for CaelumView<'_, NodeContext, MeasureFunction>
where
    MeasureFunction:
        FnMut(Size<Option<f32>>, Size<AvailableSpace>, NodeId, Option<&mut NodeContext>, &Style) -> Size<f32>,
{
...
impl<NodeContext> CaelumTree<NodeContext> {
    /// Creates a new [`CaelumTree`]
    ///
    /// The default capacity of a [`CaelumTree`] is 16 nodes.
    #[must_use]
    pub fn new() -> Self {
        Self::with_capacity(16)
    }

    /// Creates a new [`CaelumTree`] that can store `capacity` nodes before reallocation
    #[must_use]
    ...
    pub fn new_leaf(&mut self, layout: Style) -> CaelumResult<NodeId> {
    ...
    pub fn new_leaf_with_context(&mut self, layout: Style, context: NodeContext) -> CaelumResult<NodeId> {
    ...
    pub fn new_with_children(&mut self, layout: Style, children: &[NodeId]) -> CaelumResult<NodeId> {
    ...
    pub fn remove(&mut self, node: NodeId) -> CaelumResult<NodeId> {
        let key = node.into();
    ...
    #[inline]
    pub fn set_node_context(&mut self, node: NodeId, measure: Option<NodeContext>) -> CaelumResult<()> {
        let key = node.into();
    ...
    pub fn add_child(&mut self, parent: NodeId, child: NodeId) -> CaelumResult<()> {
    ...
    pub fn insert_child_at_index(&mut self, parent: NodeId, child_index: usize, child: NodeId) -> CaelumResult<()> {
        let parent_key = parent.into();

        let child_count = self.children[parent_key].len();
        if child_index > child_count {
            return Err(CaelumError::ChildIndexOutOfBounds { parent, child_index, child_count });
        }
    ...
    pub fn set_children(&mut self, parent: NodeId, children: &[NodeId]) -> CaelumResult<()> {
        let parent_key = parent.into();
    ...
    ///
    /// The child is not removed from the tree entirely, it is simply no longer attached to its previous parent.
    pub fn remove_child_at_index(&mut self, parent: NodeId, child_index: usize) -> CaelumResult<NodeId> {
        let parent_key = parent.into();
        let child_count = self.children[parent_key].len();
        if child_index >= child_count {
            return Err(CaelumError::ChildIndexOutOfBounds { parent, child_index, child_count });
    ...
    pub fn remove_children_range<R>(&mut self, parent: NodeId, range: R) -> CaelumResult<()>
    where
    ...
    #[inline]
    pub fn child_at_index(&self, parent: NodeId, child_index: usize) -> CaelumResult<NodeId> {
        let parent_key = parent.into();
        let child_count = self.children[parent_key].len();
        if child_index >= child_count {
            return Err(CaelumError::ChildIndexOutOfBounds { parent, child_index, child_count });
    ...
    pub fn children(&self, parent: NodeId) -> CaelumResult<Vec<NodeId>> {
        Ok(self.children[parent.into()].clone())
    }

    /// Sets the [`Style`] of the provided `node`
    #[inline]
    pub fn set_style(&mut self, node: NodeId, style: Style) -> CaelumResult<()> {
    ...
    #[inline]
    pub fn style(&self, node: NodeId) -> CaelumResult<&Style> {
        Ok(&self.nodes[node.into()].style)
    }

    /// Return this node layout relative to its parent
    #[inline]
    pub fn layout(&self, node: NodeId) -> CaelumResult<&Layout> {
        if self.config.use_rounding {
    ...
    pub fn mark_dirty(&mut self, node: NodeId) -> CaelumResult<()> {
    ...
    #[inline]
    pub fn dirty(&self, node: NodeId) -> CaelumResult<bool> {
    ...
    /// Updates the stored layout of the provided `node` and its children
    pub fn compute_layout(&mut self, node: NodeId, available_space: Size<AvailableSpace>) -> Result<(), CaelumError> {
    ...
    pub fn print_tree(&mut self, root: NodeId) {
        crate::engine::caelum::util::print_tree(self, root)
    }
...
mod tests {
    ...
    use super::*;
    use crate::engine::caelum::style::{Dimension, Display, FlexDirection};
    use crate::engine::caelum::style_helpers::*;
    use crate::engine::caelum::util::sys;
    ...
    fn new_should_allocate_default_capacity() {
        ...
        let Caelum: CaelumTree<()> = CaelumTree::new();

        assert!(Caelum.children.capacity() >= DEFAULT_CAPACITY);
        assert!(Caelum.parents.capacity() >= DEFAULT_CAPACITY);
        assert!(Caelum.nodes.capacity() >= DEFAULT_CAPACITY);
    }

    #[test]
    fn test_with_capacity() {
        const CAPACITY: usize = 8;
        let Caelum: CaelumTree<()> = CaelumTree::with_capacity(CAPACITY);

        assert!(Caelum.children.capacity() >= CAPACITY);
        assert!(Caelum.parents.capacity() >= CAPACITY);
        assert!(Caelum.nodes.capacity() >= CAPACITY);
    }
    ...
    fn remove_children_range() {
        ...
        for child in [child1, child2] {
            assert_eq!(Caelum.parent(child), None);
        }
    }

    // Related to: https://github.com/DioxusLabs/Caelum/issues/510
    #[test]
    ...
    fn test_child_at_index() {
        ...
        assert!(if let Ok(result) = Caelum.child_at_index(node, 2) { result == child2 } else { false });
    }
    #[test]
    fn test_child_count() {
        let mut Caelum: CaelumTree<()> = CaelumTree::new();
        ...
        assert!(Caelum.child_count(child1) == 0);
    }
    ...
    fn compute_layout_should_produce_valid_result() {
        let mut Caelum: CaelumTree<()> = CaelumTree::new();
        let node_result = Caelum.new_leaf(Style {
