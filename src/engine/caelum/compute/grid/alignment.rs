...
use crate::engine::caelum::compute::common::alignment::{apply_alignment_fallback, compute_alignment_offset};
use crate::engine::caelum::geometry::{InBothAbsAxis, Line, Point, Rect, Size};
use crate::engine::caelum::style::{AlignContent, AlignItems, AlignSelf, AvailableSpace, CoreStyle, GridItemStyle, Overflow, Position};
use crate::engine::caelum::tree::{Layout, LayoutPartialTreeExt, NodeId, SizingMode};
...
pub(super) fn align_tracks(
    ...
    let is_safe = true; // Safe alignment active: prevent overflow
...
pub(super) fn align_item_within_area(
    ...
    let alignment_based_offset = match alignment_style {
        // Treated as "start" (baseline fallback implemented)
