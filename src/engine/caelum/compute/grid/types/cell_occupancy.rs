...
use crate::engine::caelum::compute::grid::OriginZeroLine;
use crate::engine::caelum::geometry::AbsoluteAxis;
...
impl<T: Default + Clone> Grid<T> {
    ...
    fn from_vec(data: Vec<T>, cols: usize) -> Self {
        ...
        let maybe_index = match track_type {
            AbsoluteAxis::Horizontal => {
                if track_computed_index < 0 || track_computed_index >= self.inner.rows() as i16 {
                    ...
                    self.inner.iter_row(track_computed_index as usize).collect::<Vec<_>>().into_iter().rposition(|item| *item == kind)
                }
            }
            AbsoluteAxis::Vertical => {
                if track_computed_index < 0 || track_computed_index >= self.inner.cols() as i16 {
                    ...
                    self.inner.iter_col(track_computed_index as usize).collect::<Vec<_>>().into_iter().rposition(|item| *item == kind)
                }
            }
        };
        ...
        let maybe_index = match track_type {
            AbsoluteAxis::Horizontal => {
                if track_computed_index < 0 || track_computed_index >= self.inner.rows() as i16 {
                    ...
                    self.inner.iter_row(track_computed_index as usize).collect::<Vec<_>>().into_iter().position(|item| *item == kind)
                }
            }
            AbsoluteAxis::Vertical => {
                if track_computed_index < 0 || track_computed_index >= self.inner.cols() as i16 {
                    ...
                    self.inner.iter_col(track_computed_index as usize).collect::<Vec<_>>().into_iter().position(|item| *item == kind)
                }
            }
        };
