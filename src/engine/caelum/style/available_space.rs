...
use crate::engine::caelum::{
    prelude::{FromLength, CaelumMaxContent, CaelumMinContent, CaelumZero},
    sys::abs,
    Size,
};
...
impl CaelumZero for AvailableSpace {
    const ZERO: Self = Self::Definite(0.0);
}
impl CaelumMaxContent for AvailableSpace {
    const MAX_CONTENT: Self = Self::MaxContent;
}
impl CaelumMinContent for AvailableSpace {
