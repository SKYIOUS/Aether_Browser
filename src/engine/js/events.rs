use super::js_bridge::{EventListenerEntry, JsBridge};
...
impl JsBridge {
    ...
    pub fn remove_event_listener(&mut self, node_id: u32, event_type: String, source: String) {
        self.event_listeners.retain(|e| {
            !(e.node_id == node_id && e.event_type == event_type && e.source == source)
        });
    }
    ...
    pub fn get_event_listeners_bubbling(
        &self,
        node_id: u32,
        event_type: &str,
    ) -> Vec<(String, u32)> {
