...
pub fn register_default_callbacks(vm: &mut korlang::VirtualMachine) {
    vm.register_native(
        "String.split",
        Arc::new(|args: &[korlang::Value]| {
            if let (Some(korlang::Value::String(s)), Some(korlang::Value::String(sep))) =
                (args.first(), args.get(1))
            {
                let parts: Vec<korlang::Value> = s
                    .split(sep)
                    .map(|p| korlang::Value::String(p.to_string()))
                    .collect();
    ...
    vm.register_native(
        ...
        Arc::new(|args: &[korlang::Value]| {
            if let (
                Some(korlang::Value::String(s)),
                Some(korlang::Value::String(from)),
                Some(korlang::Value::String(to)),
