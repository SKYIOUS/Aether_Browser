...
const SUGGESTED: [(&str, &str, &str); 2] = [
    (
        ...
        "Synthesize current workspace with Aether AI",
    ),
    (
        "◫",
        "Switch to 'Rust Backend' Workspace",
        "4 active tabs · Last seen 2h ago",
    ),
];
...
impl PaletteScreen {
    ...
    pub fn view(&self) -> Element<'_, PaletteMessage> {
        ...
        let overlay = container(column![Space::with_height(80), panel,])
            .width(Length::Fill)
            .height(Length::Fill)
    ...
    fn palette_inner(&self) -> Element<'_, PaletteMessage> {
        ...
        let search = container(
            row![
                ...
                button(text("Esc").size(11).color(C::muted()).font(iced::Font {
        ...
        // Suggested actions
        let suggested_label = text("Suggested Actions")
            .size(10)
            .color(C::dim())
            .font(iced::Font {
                ...
                .map(|(i, (icon, title, subtitle))| {
                    button(
                        row![
                            ...
                            column![
                                text(*title).size(13).color(C::fg()).font(iced::Font {
        ...
        let history_label = text("History").size(10).color(C::dim()).font(iced::Font {
            weight: iced::font::Weight::Bold,
            ..Default::default()
        });
        ...
        let footer = container(
            row![
                ...
                row![
                    text("↵").size(14).color(C::dim()),
                    text("Select").size(10).color(C::muted()).font(iced::Font {
                        family: iced::font::Family::Monospace,
                        weight: iced::font::Weight::Bold,
                        ..Default::default()
                    }),
