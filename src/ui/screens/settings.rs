...
// Accent swatches offered in Personalization; indices index into this array,
// and the chosen rgb triple is what gets persisted to VayuSettings.
const ACCENT_SWATCHES: [[f32; 3]; 3] = [
    [0.647, 0.788, 1.000],
    [1.000, 0.706, 0.671],
    [0.827, 0.737, 0.988],
];
...
impl SettingsScreen {
    pub fn new() -> Self {
        ...
        // Reflect the persisted theme in the swatch selection and make the
        // runtime palette match before this screen can render.
        let accent_selected = ACCENT_SWATCHES
            .iter()
            .position(|[r, g, b]| {
                (r - settings.accent_color.r).abs() < f32::EPSILON
                    && (g - settings.accent_color.g).abs() < f32::EPSILON
                    && (b - settings.accent_color.b).abs() < f32::EPSILON
            })
            .unwrap_or(0);
...
impl SettingsScreen {
    pub fn update(&mut self, msg: SettingsMessage) -> Task<SettingsMessage> {
        match msg {
            ...
            SettingsMessage::AccentSelected(i) => {
                self.accent_selected = i;
                let [r, g, b] = ACCENT_SWATCHES[i];
                self.settings.accent_color = AccentColor { r, g, b };
                crate::ui::style::set_palette(self.settings.dark_mode, &self.settings.accent_color);
                self.changed = true;
            }
            SettingsMessage::ToggleDarkMode => {
    ...
    fn dark_mode_card(&self) -> Element<'_, SettingsMessage> {
        let on = self.settings.dark_mode;
        let toggle_color = if on { C::accent() } else { C::muted() };
        let toggle_text = if on { "ON" } else { "OFF" };
        let left = column![
