...
pub fn dev_console_overlay(screen: &super::BrowserScreen) -> Element<'_, super::BrowserMessage> {
    ...
    let content: Element<'_, super::BrowserMessage> = match screen.dev_tools_tab {
        DevToolsTab::Console => {
            let errors: Vec<Element<'_, super::BrowserMessage>> = screen.js_errors.iter().rev().take(50).map(|e| {
                text(format!("> {}", e)).size(12).color(Color::WHITE).into()
            }).collect();
            if errors.is_empty() {
                text("No console output").size(12).color(Color::from_rgba(1.0, 1.0, 1.0, 0.5)).into()
            } else {
        ...
        DevToolsTab::Elements => {
            ...
            let items: Vec<Element<'_, super::BrowserMessage>> = screen.styled_elements.iter().enumerate().take(200).map(|(i, el)| {
        ...
        DevToolsTab::Network => {
            let items: Vec<Element<'_, super::BrowserMessage>> = screen.network_requests.iter().rev().take(100).map(|req| {
                text(format!("> {}", req)).size(11).color(Color::from_rgba(1.0, 1.0, 1.0, 0.7)).into()
            }).collect();
            let list: Element<'_, super::BrowserMessage> = if items.is_empty() {
                text("No network requests logged").size(12).color(Color::from_rgba(1.0, 1.0, 1.0, 0.5)).into()
            } else {
...
pub fn dev_console_tabs(current: &DevToolsTab) -> Element<'_, super::BrowserMessage> {
    let make = |label: &'static str, tab: DevToolsTab| {
        ...
        button(text(label).size(12).color(fg))
            ...
            .style(move |_, status| {
                let bg = if active {
                    Some(iced::Background::Color(Color::from_rgba(0.25, 0.5, 0.9, 0.15)))
                } else {
                    match status {
                        iced::widget::button::Status::Hovered => Some(iced::Background::Color(Color::from_rgba(1.0, 1.0, 1.0, 0.08))),
                        _ => None,
                    }
                };
    ...
    let tab_row = row![
        ...
        button(text("\u{00D7}").size(14).color(Color::from_rgba(1.0, 1.0, 1.0, 0.5)))
